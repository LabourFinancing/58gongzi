...
document.write (' <DIV align=center id=\"softkeyboard\" name=\"softkeyboard\" style=\"position:absolute; left:600px; top:300px; width:517px; z-index:180;display:none\">');
...
document.write (' <tr> ');
document.write (' <td align=\"left\" bgcolor=\"#FF9900\" align=\"center\"> <INPUT class=td1b type=password size=20 value=\"\" id=\"password1\" name=\"password1\"> ');
document.write (' <INPUT class=button type=button value=确定 name=\"Submit3\" onclick=\"OverInput(curEditName);\"> <INPUT class=button type=reset value=重新输入 name=\"Submit23\"> ');
...
document.write (' <input name=\"showCapsLockValue\" typ, , e=reset value=\"当前是小写 \"></td>');
document.write (' </tr>');
document.write (' <tr align=\"left\" valign=\"middle\"> ');
document.write (' <td> ');
...
function addValue(newValue)
{
	...
	if (CapsLockValue==0){
		...
		pwd.value += newValue.toUpperCase();
}
// 实现BackSpace键的功能
function setpassvalue()
{
	var longnum=document.getElementById('password1').value.length;
	var num;
	num=document.getElementById('password1').value.substr(0,longnum-1);
	document.getElementById('password1').value=num;
}
// 输入完毕
function OverInput(theForm)
{
	var f = document.forms["Calc"];
	var sp = theForm.split(".");
	var log = document.forms[sp[0]];
	var pd = log.pwd;
	pd.value = f.password1.value;
...
function setCapsLock()
{
	...
	{
		CapsLockValue=1;
		f.showCapsLockValue.value="当前是大写 ";
	}else{
		CapsLockValue=0;
		f.showCapsLockValue.value="当前是小写 ";
	}
}
